...
performance <- map_dfr(folds$splits,
       function(x){
         ...
         y_pred <- predict(rpart(spam~., data = x.train, method="class"), newdata = x.test, type = "class")
         CART_class <- mean(y_pred == x.test$spam)
         ...
         tibble(Tree=Tree, Bagging=Bagging, RandomForest=RandomForest, LDA=LDA, LogReg=LogReg, Bayes=Bayes, CART_class=CART_class)
