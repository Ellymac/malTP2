...
performance = map.dfr(folds$splits,
                      function(x) {
                        x.train <- 
                      })

Bagging <- mean(ypred == x.test$spam)
ypred <- predict(randomForest(spam~., data=x.train),newdata=x.test,type="class");
RandomForest <- mean(ypred == x.test$spam)
ypred <- predict(lda(spam~.,data=y.train),newdata=x.test)$class
LDA <- mean(ypred == x.test$spam)
...
ggplot(data~performance,aes(x=Algorithm, y=Error,col=Algorithm))
geom_boxplot()
```
